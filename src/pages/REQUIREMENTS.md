...
### `Notifications.vue` — 通知历史页

**路由：** `/notifications`

**功能：**

- 展示所有历史通知（`NDataTable`，虚拟滚动 + 列排序启用）
- 通知类型标签：信息 / 成功 / 警告 / 错误（`NTag` 颜色区分）
- 通知来源：整合器 / 系统 / 渠道
- 单条标记已读 / 全部标记已读
- 一键清空全部通知
- 支持按类型、来源、时间、已读状态排序
...
- 通知：`useNotificationStore`（通知列表、分页、已读管理）
...
- 渠道未配置时显示警告提示（`NAlert`）
- 空状态引导创建（`NEmpty` + 创建按钮）
- **通知测试按钮**（页头 `NButton secondary`）：点击直接触发 WebUI 通知弹窗，用于验证通知功能是否正常
